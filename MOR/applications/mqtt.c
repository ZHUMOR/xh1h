...
int example_publish(void *handle)
{
    ...
    char           *topic = NULL;
    int             topic_len = 0;
    char           *payload = "{\"params\":{\"temp\":%d}}";

    /*------------------处理温度数据----------------*/
    char pay_load[70];
    temperature=(int)temperature;
    rt_kprintf("send temp:%d",(int)temperature);

    HAL_Snprintf(pay_load,80,payload,(int)temperature);

    rt_kprintf("Final pay_load: %s\n", pay_load);
    /*-----------------------处理结束------------*/
    topic_len = strlen(fmt) + strlen(DEMO_PRODUCT_KEY) + strlen(DEMO_DEVICE_NAME) + 1;
    topic = HAL_Malloc(topic_len);
    if (topic == NULL) {
    ...
    memset(topic, 0, topic_len);
    HAL_Snprintf(topic, topic_len, fmt, DEMO_PRODUCT_KEY, DEMO_DEVICE_NAME);
    res = IOT_MQTT_Publish_Simple(0, topic, IOTX_MQTT_QOS0, pay_load, strlen(pay_load));
    if (res < 0) {
...
{
    rt_thread_t thread;
    thread = rt_thread_create("ali",mqtt_example_main , RT_NULL, 4096, 11, 10);
    if (thread == RT_NULL)
    {
        return -RT_ERROR;
    }
